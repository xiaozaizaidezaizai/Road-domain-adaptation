...
def train():
    ...
    train_dataset = RoadUDADataset(
        ...
        is_mass_target=True  # 开启 Massachusetts 模式
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % EVAL_INTERVAL == 0 or (epoch + 1) == NUM_EPOCHS:
            ...
            print(f" >> [Target Inv]   IoU: {iou_t_inv:.4f}")
            ...
            # =========================================================
            # [新增功能] 强制保存当前评估轮次的权重 (文件名带 epoch)
            # =========================================================
            current_save_name = f'checkpoint_epoch_{epoch + 1}_iou_{iou_t_inv:.4f}.pth'
            current_save_path = os.path.join(SAVE_DIR, current_save_name)
            ...
            torch.save({
                ...
                'optimizer_state': optimizer.state_dict(),
                'target_iou': iou_t_inv
            }, current_save_path)
            print(f" -> Saved current evaluation weights to: {current_save_name}")
...
if __name__ == '__main__':
    ...
    SAVE_DIR = f'./checkpoints/MASS_ablation_adv_mix3_{TIME_STR}'
