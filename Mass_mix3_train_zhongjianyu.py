...
TARGET_ROOT = r"C:\Users\Administrator\Desktop\RoadData\Massachusetts" # 你的新路径
# SOURCE_LIST_NAME = "source_domain_list.txt"
SOURCE_LIST_NAME = "train.txt"
TARGET_LIST_NAME = "train_cleaned.txt"
# 训练超参数
...
def calc_prototype_loss_refined(feats_mix, feats_s, feats_t, mask_mix, pred_mix, threshold=0.9):
    """
    带【伪标签修正】的原型解耦对齐损失

    参数:
        feats_mix: 中间域特征列表 (多尺度)
        feats_s:   源域特征列表
        feats_t:   目标域特征列表
        mask_mix:  混合时的二值掩码 (1=源域, 0=目标域)
        pred_mix:  【新增】模型对中间域图片的预测结果 (Logits 或 Prob), 形状 (B, 1, H, W)
        threshold: 【新增】伪标签阈值，置信度高于此值的目标域区域被视为"潜在道路"，不参与背景对齐。
    """
    loss = 0.0
    valid_layers = 0

    # 确保预测结果不传导梯度 (当作固定的参考)
    pred_mix = pred_mix.detach()
    # 如果 pred_mix 是 logits (未经过 sigmoid)，先转为概率
    if pred_mix.max() > 1.0 or pred_mix.min() < 0.0:
        pred_prob = torch.sigmoid(pred_mix)
    else:
        pred_prob = pred_mix
    ...
    for f_m, f_s, f_t in zip(feats_mix, feats_s, feats_t):
        ...
        # 1. 下采样 Mask (源域真值)
        mask_resized = F.interpolate(mask_mix, size=(H, W), mode='nearest')

        # 2. 【新增】下采样预测结果，并生成伪标签
        # 我们需要知道在当前分辨率下，模型认为哪里是路
        pred_resized = F.interpolate(pred_prob, size=(H, W), mode='bilinear', align_corners=True)
        # 生成高置信度伪标签 (1表示模型非常确信这是路)
        pseudo_label = (pred_resized > threshold).float()

        # ---------------------------
        # A. 前景原型 (保持不变)
        # ---------------------------
        # 只利用 mask_mix=1 的区域 (绝对纯净的源域道路)
        if mask_resized.sum() > 0:
            ...
            proto_m_fg = feat_m_fg_sum / mask_fg_count
            proto_s_fg = feat_s_fg_sum / mask_fg_count

            proto_s_fg = proto_s_fg.detach()  # 冻结源域

            proto_m_fg_norm = F.normalize(proto_m_fg, p=2, dim=1)
            proto_s_fg_norm = F.normalize(proto_s_fg, p=2, dim=1)
        ...
        # B. 背景原型 (【核心修改区域】)
        # ---------------------------
        # 原始背景掩码：所有非源域区域 (包含了 目标域背景 + 目标域道路)
        mask_bg_raw = 1 - mask_resized

        # 【修正步骤】：从背景中挖掉"疑似目标域道路"
        # 逻辑：如果是背景(1) 且 模型预测不是路(0) -> 1 * 1 = 1 (保留，这是真背景)
        # 逻辑：如果是背景(1) 但 模型预测是路(1)   -> 1 * 0 = 0 (剔除，这是目标域道路，保护起来！)
        mask_bg_refined = mask_bg_raw * (1 - pseudo_label)

        mask_bg_count = mask_bg_refined.sum(dim=(2, 3)) + 1e-6

        loss_bg = 0.0
        # 只有当还有背景像素剩余时才计算 (防止全被挖空)
        if mask_bg_count.min() > 0:
            # 注意：这里我们用【修正后】的掩码提取混合图背景
            feat_m_bg_sum = (f_m * mask_bg_refined).sum(dim=(2, 3))

            # 目标域特征 feats_t 对应的掩码也建议修正，或者直接用全图平均
            # 这里为了严谨，我们假设目标域图片全是背景 (或者同样用伪标签过滤，简单起见用修正掩码过滤目标域特征也是合理的)
            feat_t_bg_sum = (f_t * mask_bg_refined).sum(dim=(2, 3))

            proto_m_bg = feat_m_bg_sum / mask_bg_count
            proto_t_bg = feat_t_bg_sum / mask_bg_count

            proto_t_bg = proto_t_bg.detach()  # 冻结目标域

            proto_m_bg_norm = F.normalize(proto_m_bg, p=2, dim=1)
            proto_t_bg_norm = F.normalize(proto_t_bg, p=2, dim=1)
...
def train():
    ...
    train_dataset = RoadUDADataset(
        ...
        source_list_name=SOURCE_LIST_NAME, target_list_name=TARGET_LIST_NAME,
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (img_s, mask_s, img_t) in enumerate(pbar):
            ...
            if epoch < WARMUP_EPOCHS:
                ...
                # --- [新增代码] 计算特征层面的区域解耦一致性损失 ---
                # 将混合特征、源域特征、目标域特征、以及混合Mask传入
                loss_feat_mix = calc_prototype_loss_refined(
                    feats_mix, feats_s, feats_inv_t, mask_mix,pred_mix
                )
        ...
        if (epoch + 1) % EVAL_INTERVAL == 0 or (epoch + 1) == NUM_EPOCHS:
            ...
            SAVE_CYCLE = 2  # 设置为每 2 次评估保存一次

            if (epoch + 1) % (EVAL_INTERVAL * SAVE_CYCLE) == 0:
                periodic_name = f'checkpoint_epoch_{epoch + 1}_iou_{iou_t_inv:.4f}.pth'
                save_path = os.path.join(SAVE_DIR, periodic_name)

                torch.save({
                    'epoch': epoch,
                    'enc_invariant': enc_invariant.state_dict(),
                    'decoder': decoder.state_dict(),  # 记得保存解码器
                    'optimizer_state': optimizer.state_dict()
                }, save_path)

                print(f" >> [Periodic Save] Checkpoint saved: {periodic_name}")

if __name__ == '__main__':
    os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"
    TIME_STR = datetime.now().strftime("%Y%m%d_%H%M%S")
    # 可以在文件夹名字里加上 ablation 方便区分
    SAVE_DIR = f'./checkpoints/MASS_ablation_adv_mix3_zhongjianyu_{TIME_STR}'
