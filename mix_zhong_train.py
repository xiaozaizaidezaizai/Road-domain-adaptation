...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (img_s, mask_s, img_t) in enumerate(pbar):
            ...
            if epoch < WARMUP_EPOCHS:
                ...
                if epoch < 50:
                    ...
                    LOW_THRESH = 0.1
                    LAMBDA_MIX = 1
                    LAMBDA_FEAT_MIX = 0.1           # D. 构建硬阈值伪标签 (Hard Thresholding Target)
                with torch.no_grad():
        ...
        # 1. 每一轮都执行的部分：目标域评测
        if (epoch + 1) % 1 == 0:  # 这里的 1 可以根据需要改成变量 TARGET_EVAL_INTERVAL
            print(f"\n[Eval] Epoch {epoch + 1} Target Domain Evaluation...")
        ...
        # 2. 每 10 轮执行一次的部分：源域评测
        SOURCE_EVAL_INTERVAL = 10
        if (epoch + 1) % SOURCE_EVAL_INTERVAL == 0 or (epoch + 1) == NUM_EPOCHS:
            print(f"\n[Eval] Epoch {epoch + 1} Source Domain Evaluation...")
            ...
            # 记录历史数据（注意：如果历史记录需要对齐，源域不评测的轮次可以填入 None 或上一次的值）
            history['source_iou_inv'].append(iou_s_inv)
        else:
            # 保持 history 列表长度一致，防止绘图时报错
            history['source_iou_inv'].append(None)

            # 3. 每轮保存一次 JSON 记录（包含最新的 Target 结果）
        with open(os.path.join(SAVE_DIR, 'metrics.json'), 'w') as f:
            json.dump(history, f, indent=4)
...
if __name__ == '__main__':
    ...
    SAVE_DIR = f'./checkpoints/ablation_mix_zhong_meiyici{TIME_STR}'
