...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (img_s, mask_s, img_t) in enumerate(pbar):
            ...
            # =============================================================
            # [修改开始] 消融实验：禁用中间域混合模块 & 特征对齐
            # =============================================================

            # 1. 强制将中间域相关的 Loss 和 权重 设为 0
            ...
            # 2. 对抗损失的处理 (Adv)
            #    消融实验通常保留对抗学习作为 Baseline (Seg + Adv)
            #    如果你想做纯源域 (Source Only)，把 LAMBDA_ADV 也设为 0
            if epoch < WARMUP_EPOCHS:
                loss_adv_total = torch.tensor(0.0).to(DEVICE)
                LAMBDA_ADV = 0.0
            else:
                LAMBDA_ADV = 0.01  # 保持对抗训练权重

                # --- [已注释] 中间域混合模块 & 特征对齐 ---
                # 原来的 img_mix, feats_mix, pred_mix 全部注释掉，避免计算消耗显存和时间
                """
                # A. 准备蒙版
                if mask_s.dim() == 3: mask_mix = mask_s.unsqueeze(1).float()
                else: mask_mix = mask_s.float()
            ...
            # --- 5. 总损失 ---
            # 此时 Mix 和 Feat 相关的系数都是 0，不会影响梯度
...
if __name__ == '__main__':
    ...
    SAVE_DIR = f'./checkpoints/MASS_Ablation_NoMix_AdvOnly_danceng_{TIME_STR}'
