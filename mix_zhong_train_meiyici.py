...
history = {
    ...
    # Target 域指标 (Inv 分支)
    'target_iou_inv': [],
    'target_precision_inv': [],
    'target_recall_inv': [],
    'target_f1_inv': [],
    # Source 域指标 (Inv 分支)
    'source_iou_inv': [],
    'source_precision_inv': [],
    'source_recall_inv': [],
    'source_f1_inv': []
}
...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        metrics_t = evaluator.evaluate(target_val_loader, desc="Eval Target")
        res_t = metrics_t['Inv']  # 提取消融实验关注的 Inv 分支指标

        # 记录目标域详细指标
        history['epoch'].append(epoch + 1)
        history['train_loss'].append(avg_loss)
        history['target_iou_inv'].append(res_t['IoU'])
        history['target_precision_inv'].append(res_t['Precision'])
        history['target_recall_inv'].append(res_t['Recall'])
        history['target_f1_inv'].append(res_t['F1'])

        print(f" >> [Target Inv] IoU: {res_t['IoU']:.4f} | F1: {res_t['F1']:.4f}")
        ...
        if (epoch + 1) % SOURCE_EVAL_INTERVAL == 0 or (epoch + 1) == NUM_EPOCHS:
            ...
            history['source_iou_inv'].append(res_s['IoU'])
            history['source_precision_inv'].append(res_s['Precision'])
            history['source_recall_inv'].append(res_s['Recall'])
            history['source_f1_inv'].append(res_s['F1'])
            print(f" >> [Source Inv] IoU: {res_s['IoU']:.4f}")
        else:
            # 关键：不评测源域时，填充 None 以保证 JSON 数据对齐
            history['source_iou_inv'].append(None)
            history['source_precision_inv'].append(None)
            history['source_recall_inv'].append(None)
            history['source_f1_inv'].append(None)

        # 3. 保存 JSON 记录
        with open(os.path.join(SAVE_DIR, 'metrics.json'), 'w') as f:
            json.dump(history, f, indent=4)

        # 4. 检查并保存最佳模型（基于目标域 IoU）
        if res_t['IoU'] > best_target_iou:
            best_target_iou = res_t['IoU']
            save_path = os.path.join(SAVE_DIR, 'best_model.pth')
            torch.save({
                'epoch': epoch,
                'enc_invariant': enc_invariant.state_dict(),
                'decoder': decoder.state_dict(),
                'best_iou': best_target_iou,
                'history': history  # 建议把 history 也存进 checkpoint
            }, save_path)
            print(f" !! New Best Model Saved (Inv IoU: {best_target_iou:.4f})")
...
if __name__ == '__main__':
    ...
    SAVE_DIR = f'./checkpoints/ablation_mix_zhong_meiyici_2_{TIME_STR}'
